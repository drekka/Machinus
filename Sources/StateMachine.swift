...
public typealias DidTransition<S> = (_ fromState: S, _ toState: S) async -> Void where S: StateIdentifier
...
public actor StateMachine<S> where S: StateIdentifier {
    ...
    func transition(toState: StateConfig<S>, didExit: DidExit<S>?, didEnter: DidEnter<S>?) async -> StateConfig<S> {
        ...
        await didExit?(toState.identifier)
        await didEnter?(fromState.identifier)
        await didTransition?(fromState.identifier, toState.identifier)
        return fromState
    }
}
