...
public typealias DynamicTransition<S> = @Sendable () -> S where S: StateIdentifier
...
public typealias TransitionBarrier<S> = @Sendable (S) -> BarrierResponse<S> where S: StateIdentifier
...
public typealias DidEnterState<S> = @Sendable (_ from: S, _ to: S) -> Void where S: StateIdentifier

/// An action set on a state that is executed when the machine transitions away from the state.
/// - parameter from: The previous state of the machine.
/// - parameter to: The new state of the machine.
public typealias DidExitState<S> = @Sendable (_ from: S, _ to: S) -> Void where S: StateIdentifier

/// Set on the state machine and called after each successful transition.
///
/// - parameter from: The previous state of the machine.
/// - parameter to: The new state of the machine.
public typealias DidTransition<S> = @Sendable (_ from: S, _ to: S) -> Void where S: StateIdentifier
