...
public typealias TransitionBarrier<S> = @Sendable (S) async -> BarrierResponse<S> where S: StateIdentifier
...
public typealias DidEnterState<S> = @Sendable (_ from: S, _ to: S) async -> Void where S: StateIdentifier

/// An action set on a state that is executed when the machine transitions away from the state.
/// - parameter from: The previous state of the machine.
/// - parameter to: The new state of the machine.
public typealias DidExitState<S> = @Sendable (_ from: S, _ to: S) async -> Void where S: StateIdentifier

/// Set on the state machine and called after each successful transition.
///
/// - parameter from: The previous state of the machine.
/// - parameter to: The new state of the machine.
public typealias MachineDidTransition<S> = @Sendable (_ from: S, _ to: S) async -> Void where S: StateIdentifier
